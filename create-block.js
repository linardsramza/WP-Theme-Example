...
console.log( "\n" + chalk.blue( "Let's create a block for Gutenberg! \n" ) );
...
inquirer
	.prompt([
		{
			type: 'input',
			name: 'block_title',
			message: "What's the title of the block?"
		},
		{
			type: 'list',
			name: 'block_mode',
			message: 'What mode should the block have?',
			choices: [
				'preview',
				'edit',
				'auto'
		...
		},
		{
			type: 'input',
			name: 'block_icon',
			message: 'What icon do you want to use for the block?'
		},
		{
			type: 'list',
			name: 'block_javascript',
			message: 'Should this block have a separate JavaScript file?',
			choices: [
				'Yes',
				'No'
			]
	...
	.then(a => {
		...
		var block_name = wpFeSanitizeTitle(a.block_title);

		// Create the folder
		fs.mkdir( path.join( __dirname + '/blocks/' + block_name ), { recursive: true }, (err) => {
			if (err) return console.log( err );

			// Create the files
			fs.writeFile( path.join( __dirname + '/blocks/' + block_name + '/' + block_name + '.scss' ), '', function (err) {
				if (err) return console.log( err );
			});
